...
def _estimate_support_area(mesh: trimesh.Trimesh, threshold_angle: float = 45.0) -> float:
    """
    Yüzey normallerinin Z ekseniyle açısı threshold_angle'dan büyükse
    (yani parça sarktaysa) destek gerektirir.
    """
    normals = mesh.face_normals
    # Her üçgenin Z normali negatifse ve açısı kritik eşiği aşıyorsa
    z_component = normals[:, 2]
    threshold_cos = np.cos(np.radians(180 - threshold_angle))
    overhang_mask = z_component < threshold_cos

    # Sarkık yüzlerin alanları
...
def _calculate_complexity(mesh: trimesh.Trimesh, packing_density: float, support_ratio: float) -> int:
    """
    0-100 arası kompleksite skoru.
    Düşük packing density + yüksek destek oranı + çok üçgen = yüksek kompleksite
    """
    score = 0

    # Packing density: 1'e yakın = basit blok, 0'a yakın = karmaşık
    score += int((1 - packing_density) * 40)

    # Destek oranı
    score += int(support_ratio * 30)

    # Üçgen sayısı (normalize: 100k üçgen = max)
...
def _generate_warnings(mesh, dimensions, is_watertight) -> list:
    ...
    # Çok büyük parça kontrolü (standart yazıcı sınırları)
